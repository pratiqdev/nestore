type ProxyObject<T> = {
  ...
  function createNestore<T extends Record<string | number, unknown>>(
    ...
    function get<K extends keyof T>(
      pathOrFunc: K | ((state: T) => T[K])
    ): T[K] {
      const key = typeof pathOrFunc === 'function' ? '' : pathOrFunc;
      const value = typeof pathOrFunc === 'function' ? pathOrFunc(store) : undefined;
      const path = key.toString().split('.');
      const result = path.reduce((obj:any, key) => obj[key], value || store);
      /*
